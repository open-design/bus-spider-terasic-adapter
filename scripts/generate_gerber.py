...
BUILD_DIRECTORY = os.path.abspath(PCB_DIRNAME + "/" + 'build')
...
def plot_to_directory(output_directory, temp_dir):
    ...
    with pcb_util.get_plotter(PCB_FILENAME, temp_dir) as plotter:
        ...
        layers = [
            LayerDef(pcbnew.F_Cu,       'Top.gbr'),     # медное покрытие верхнего слоя
            LayerDef(pcbnew.F_Mask,     'MaskTop.gbr'), # маска верхнего слоя
            LayerDef(pcbnew.F_SilkS,    'SilkTop.gbr'), # тексты на верхней стороне платы
            LayerDef(pcbnew.B_Cu,       'Bottom.gbr'),  # медное покрытие нижнего слоя
            LayerDef(pcbnew.B_Mask,     'MaskBot.gbr'), # маска нижнего слоя
            LayerDef(pcbnew.B_SilkS,    'SilkBot.gbr'), # тексты на нижней стороне платы
            LayerDef(pcbnew.Edge_Cuts,  'Border.gbr'),  # контур платы
                                      # 'drl'           # сверловка
        ]
        ...
        desired_drill_file = os.path.join(output_directory, '%s.drl' % (board_name,))
